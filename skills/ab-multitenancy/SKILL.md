...
| `AgentDefinitionEntity` | `Guid Id` | Agent registration — `Name`, `Instructions`, JSON collection columns, `MetadataJson` (persona + enabled tools) |
...
- **`ab-agent-registration`** — register agents **per tenant** at runtime with a custom `IAsyncAgentRegistry` via the skill's [Dynamic Agent Registration](../ab-agent-registration/SKILL.md#dynamic-agent-registration) section (builds on the AsyncLocal tenant context + proxy pattern above). Per-tenant registries are the highest-risk case for the renderer-thread deadlock, because each tenant's first lookup is a cold cache hit that goes to the database: implement the **async** members and never let the synchronous ones touch EF.
