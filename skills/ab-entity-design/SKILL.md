...
public abstract class AgentDefinitionEntity
{
    ...
    /// The persona is user-managed instructions: store-backed registries merge it
    /// into AgentRegistration.Instructions at hydration (non-destructive).
    /// </summary>
...
> **AgentBlazor feature:** `AgentDefinitionEntity` maps to dynamic agent registration via `AgentRegistrationBuilder`. `Name` is the lookup key for `IAgentRegistry.TryGet()` and its awaited twin `IAsyncAgentRegistry.TryGetAsync()`. Persona and enabled tools are carried in `Metadata` (via `MetadataJson`). The persona (user-managed instructions) merges into `AgentRegistration.Instructions` at hydration; the enabled tools feed `IAgentRuntimeCustomizer` for tool-whitelist and user-context customization. See `ab-agent-registration` for entity-to-registration mapping and `ab-context-assembly` for the customizer integration.
...
| Persona + enabled tools in `MetadataJson` | Mirrors `AgentRegistration.Metadata` 1:1 — no dual-write; the persona merges into `Instructions` at hydration, the enabled tools feed the customizer from the hydrated registration's `Metadata` on every turn. |
...
**See also:** `ab-agent-registration` → "Entity Persistence" for the full mapping between `AgentDefinitionEntity` and `AgentRegistration`, and `ab-context-assembly` → "Agent Builder × customizer integration" for how the metadata-carried persona merges into `Instructions` at hydration and the enabled tools feed the runtime customizer.
...
| `AgentDefinitionEntity` | `AgentRegistration` / `AgentRegistrationBuilder` | Dynamic agent registration — `Name` maps to registration key, metadata-carried persona merges into `Instructions` at hydration, enabled tools feed `IAgentRuntimeCustomizer` |
