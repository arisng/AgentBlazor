...
> **⚠️ Architecture (v0.4.0):** `BaseSessionId` and `AgentName` are **consumer-side extensions** — they are NOT part of the library base entity. Consumer apps add these properties in their derived session entity subclass and manage migrations in their own DbContext. The library base `ConversationSessionEntity` provides only `Guid Id`, `string SessionId`, `string? UserId`, timestamps, and the `Turns` navigation.
