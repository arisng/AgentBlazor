...
description: "Configure the AI provider seam for AgentBlazor agents from a consumer app referencing the public AgentBlazor NuGet package — pinning ChatOptions that flow to the provider on every agent turn. Use when a model family rejects function tools or chat-completions requests (HTTP 400 naming reasoning_effort), when gpt-5.6-luna/-sol/-terra needs ReasoningEffort.None pinned, when wiring AgentBlazorRegistrationOptions.ConfigureChatOptions (v0.2.23+), when reproducing the 0.2.22 consumer wrapper workaround, when diagnosing per-provider option mapping (OpenAI vs Azure vs Ollama vs OriginAI), when pinning per-tenant options behind a proxy IChatClient, or when considering the Responses API (api.responses.write) escape hatch. Consumer-side only; changes live in the host app's Program.cs and appsettings; never edit package internals. Triggers: reasoning_effort, gpt-5.6, gpt-5.6-luna, gpt-5.6-sol, gpt-5.6-terra, function tools rejected, 400 reasoning_effort, ReasoningOptions, ReasoningEffort, ConfigureChatOptions, ConfigureOptions, ChatOptions, provider options, Responses API, api.responses.write, provider-level options, pin reasoning effort, no reasoning_effort sent."
metadata:
    version: 0.1.0
---
