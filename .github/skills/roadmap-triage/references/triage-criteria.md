...
> **Fork scope (divergence is intentional):** Every candidate is triaged **only against the fork** `arisng/AgentBlazor` (`origin`). Map candidates to the fork's roadmap and checkpoints; never to upstream. Issues/PRs/runs resident on the upstream repository are excluded from the inventory entirely — never classified as Signal/Defer, at most listed under rejected/out-of-scope with their owning repo named. Always pass an explicit `--repo arisng/AgentBlazor` to `gh` and confirm ownership before inclusion (default-repo resolution has been observed to leak upstream items into the inventory). "Pending upstream release" and all other upstream references in this file mean upstream **dependencies/libraries**, not the upstream repository `ashpeterson/AgentBlazor`. The upstream repo is a sync source only and is never used to rank or prioritize fork work.
