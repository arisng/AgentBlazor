...
| Token usage & cost persistence | ✅ | `ConversationTurn.Usage` → `demo_conversation_turns` columns (`PromptTokens`/`EstimatedCost`/rate snapshot incl. `CachedInputTokenCostPerMillion`); `DemoTokenPricing` section (cached rate 0.0075); `DemoUsageCostCalculator` shared with JSONL log; `DemoConversationTurnQuery` (batched usage + plan rollups) + session-browser chips with cache-hit % and K/M/B formatting; schema managed by EF Core code-first migrations | `ab-conversation-store` |
